...
public class sorting_stocks : MonoBehaviour
{
    ...
    public void onAddRemove_holdings(GameObject _target, int _AddOrRemove)//_target holds the current selected stock,, _AddOrRemove can only be 1 or -1 tells if stock has been added or removed
    {
        ...
        _current_index = (_target.transform.parent == transform) ? (_target.transform.GetSiblingIndex()) : (_place_holder.transform.GetSiblingIndex());
        ...
        if (_current_index - _AddOrRemove != _index_to_check)
        {
            ...
            StartCoroutine(moveTowards(_AddOrRemove));
        }

    }

    IEnumerator moveTowards(int _AddOrRemove)
    {
        ...
        target.transform.SetSiblingIndex((_AddOrRemove == 1) ? (_index_to_check + 1) : (_index_to_check));
        print("new index-" + _place_holder.transform.GetSiblingIndex());
